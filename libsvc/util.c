...
void set_kqueue_timer(int *kq, struct kevent *ke, int sec, int ident)
{
	...
	EV_SET(&e, ident, EVFILT_TIMER, EV_ADD | EV_ONESHOT, 0, sec * 1000, 0);
	i =kevent(*kq, &e, 1, NULL, 0, NULL);
	if (i == -1)
		dbg ("timer kevent!\n");
}

void unset_kqueue_timer(int *kq, struct kevent *ke, int sec, int ident)
{
	struct kevent e;
	static int i;
	EV_SET(&e, ident, EVFILT_TIMER, EV_DELETE, 0, sec * 1000, 0);
	i =kevent(*kq, &e, 1, NULL, 0, NULL);
	if (i == -1)
		dbg ("timer kevent!\n");
}
